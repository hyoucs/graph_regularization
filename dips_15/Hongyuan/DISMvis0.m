...
% feaNum stores the number of selected features
m.feaNum = [];
...
for i = 1:length(lambda1s)
    ...
    for j = 1:length(lambda2s)
        ...
        m.theta = m.wAll(:,dDim*(idx-1)+1:dDim*idx);

        figName = sprintf('%s/%6.3f_%6.3f_%d_%d_%6.3f', ...
            data.figName, opt.lambda1, opt.lambda2, opt.k, ...
            cntFea, mean(m.accuCV(idx,3:end)));
        visual(data, m, figName);
    end
end
