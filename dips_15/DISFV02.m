...
figure;
subplot(2,2,1);PlotX(m.Y,data.gnd,'','',''); title('embedded space');
subplot(2,2,2);PlotX(m.Z,data.gnd,'','',''); title('projected space');
subplot(2,2,3);stem(abs(m.theta(:,1))); title('1st principle');
if size(m.theta,2) > 1
    subplot(2,2,4);stem(abs(m.theta(:,2))); title('2nd principle');
end
