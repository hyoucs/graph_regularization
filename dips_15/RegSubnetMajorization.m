...
tildeL=data.X*(Lb + Aw)*data.X'+ 0.01*eye(nFea); %-- 0.01*C
B = max(tildeL,tildeL'); %-- check diagonals >0?
[U,S,V]=SVD_CutOff(B,1);
Bhalf=U*(S.^.5)*V';
W = diag(data.X*Dw*data.X'); %-- simplify W to be diag.mat, and thus a Vector
clear tildeL Dw
...
while niter < maxIter
    ...
    if ~any(q) %-- all entries in q are 0
       theta = zeros(nFea,1);
    else
%         theta = q/(sqrt(q'*W*q)); %-- full W
        theta = q/(sqrt(q'*diag(W)*q)); %-- diag W
    end
