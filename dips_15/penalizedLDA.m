function model = penalizedLDA(data,options,varargin)
%------------------------------------------------------------------------%
% Implement penalizedLDA following Witten and Irina paper
%   Max: v'Bv - beta*abs(v) s.t. v'Wv <=1 
%   2 steps (NOT complete updating yet): 
%  Step1: Call LDA to get get B,W, intial theta 
%  Step2: Call CoordinateAcsent which iterates the following:
%--     u = B^{.5}*v 
...
%% Step 1: LDA for B,W, initial theta 

[Y,m0] = lda(data.X',data.gnd);
% figure; subplot(1,2,1);PlotX(Y',data.gnd,'','',''); subplot(1,2,2);stem(abs(m0.eVecs(:,1)));
data.B = full(m0.B); data.W = full(m0.W); 
