function [auc, prec, tpr, fpr, thresh] = prec_rec(score, target,varargin)
...
num_thresh = -1;
hold_fig = 0;
%-- DQA: disable to always ROC plotting + output
% plot_roc = (nargout <= 0);
% plot_pr  = (nargout <= 0); %-- DQA
...
%             i=1:4:101; x=x(i); y=y(i); %-- DQA make ROC less smooth
%              size([x y])
            %plot(x(1:end),y(1:end), style,'MarkerEdgeColor','k','LineWidth',2);           
            plot(x,y,style,'LineWidth',1.2, 'MarkerSize',4);           
            axis([-0.02 1.02 -0.02 1.02]); %-- DQA adjust            
