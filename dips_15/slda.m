...
%   References
%   -------
%   [1] L. Clemmensen, T. Hastie, D. Witten and B. Ersb�ll. Sparse
...
% %
% 
% % -- p=150; n = 300; k= 3 (3 Gausses, each 100 samples)
% % --
% 
% % Fix stream of random numbers
% 
...
% c2 = mvnrnd(m2,S,nc); % class 2 data
% c3 = mvnrnd(m3,S,nc); % class 3 data
% X = [c1; c2; c3]; % training data setclc
% Y = [[ones(nc,1);zeros(2*nc,1)] [zeros(nc,1); ones(nc,1); zeros(nc,1)] [zeros(2*nc,1); ones(nc,1)]];
% 
% % test data
% c1 = mvnrnd(m1,S,nc);
...
% X_test = [c1; c2; c3];
% 
% figure; figure;PlotX(X(:,1:2)',Yc,'','',''); grid on;
% 
% % SLDA parameters
% delta = 1e-3;     % l2-norm constraint
% stop = -30;       % request 30 non-zero variables
...
% fName = cell(1,150);
% for i=1:150
%     fName(i)= {strcat('f',num2str(i))};
% end
% 
% %   save('data/3Gauss150Dim.mat','X','X_test','Yc','B','OptScore','fName');
% 
% 
% % Restore random stream
...
% end
% %
% %    data.X=X';  data.gnd=Yc';    data.X_test=X_test';    data.fName = fName;    data.B=B;    data.OptScore = OptScore;
% %    save('data/2Gauss150Dim.mat','X','X_test','Yc','B','OptScore','fName');
